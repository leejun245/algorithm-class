...
# 1. 순차 탐색 (Sequential Search) : 시간복잡도 평균 O(n) 
...
# 2. 이동 평균 탐색 (Sequential Search with Transpose) : 시간 복잡도 평균 O(n)
def sequential_search_transpose(arr, key, low=0, high=None):
    ...
    for i in range(low, high + 1): # low ~ high까지 순차 탐색
        ...
        if arr[i] == key: # 키를 찾으면
            ...
            if i > low : # 첫번쨰 원소가 아니라면
                arr[i], arr[i - 1]= arr[i - 1], arr[i]
                i -= 1 # 인덱스 한칸 앞으로
            return i, count # 인덱스와 비교 횟수 반환
    return -1, count # 못 찾으면 -1 반환

# 3. 이진 탐색 (Binary Search): 시간복잡도 O(log n)
def binary_search_iter(arr, key, low=0, high=None):
    if high is None:
        high = len(arr) - 1         # high의 기본값 처리
    count = 0                       # 비교 연산 횟수 카운트
    left, right = low, high         # 탐색 구간의 양 끝으로 초기화
    mid = (left + right) // 2       # 중간 인덱스 계산
    if arr[mid] == key:             # 중간값이 키인 경우 바로 반환  
        return mid, count
