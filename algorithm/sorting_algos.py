...
def quick_sort(arr, left, right): # 퀵정렬
    ...
    def partition(A, left, right): # 분할 함수 내부구현
        pivot = A[left] # 피벗 시작요소로 설정
        low = left + 1 # 피벗 다음부터 시작해서 피벗보다 작은 값 찾기
        high = right # 피벗보다 큰 값 찾기
        
        while low <= high: # 엇갈릴 때까지 반복
            while low <= right and A[low] <= pivot: # 크기안에서 피벗조다 작거나 같은 값 찾기
                low += 1 # 오른쪽으로 이동
            while high >= left and A[high] > pivot: # 피벗보다 크거나 같은 값 찾기
                high -= 1 # 왼쪽으로 이동
...
def radix_sort(arr, bucket_size = 10): # 기수 정렬
    ...
    while max_val // factor > 0: # 최대값의 모든 자리수에 대해 반복
        for i in range(n): # 배열의 모든 원소에 대해
            ...
            queues[digit].append(arr[i]) # 해당 버킷 숫자에 원소 추가

        idx = 0 # 배열 인덱스 초기화
        for b in range(bucket_size): # 각 버킷에서 원소 꺼내기
            while queues[b]: # 버킷이 비어있지 않으면
                arr[idx] = queues[b].popleft() # 원소 꺼내기 배열에 저장
                idx += 1 # 배열 인덱스 증가

        print(f"기수정렬 중간 단계 (factor={factor}): {arr[:100]} ...") # 중간 단계 출력 (처음~100개)
        factor += bucket_size # 다음 자리수로 이동 - 10배 증가
    return arr
        
def list_sorted(arr): # 리스트의 sort 메소드
    return arr.sort(reverse = False) # 깁너 어름차순 정렬

def python_sorted(arr): # 내장 정렬 함수
    return sorted(arr) # 기본 오름차순 정렬

def lambda_sorted(arr): # 람다 함수 활용 정렬
    return sorted(arr, key = lambda x : x, reverse = False) # 복잡한 객체(튜플, 딕셔너리)를 정렬
    # ex : x {"kim", 90}, {"pack", 80}, lambda x : x[1] 점수로 정렬
