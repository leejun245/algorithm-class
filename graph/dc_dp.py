...
def power_dc(x, n):
    ...
    if n == 1: # 종료 조건
        ...
        return power_dc(x*x, n//2) # 재귀호출 - 절반 크기로 감소 - n(logn)
    else : # n이 홀수
        return x * power_dc(x*x, (n-1)//2) # 재귀호출 - 절반 크기로 감소 - n(logn)
    
# 테스트 코드
x = 2.0
n = 10
print(f"억지기법_ x의 n거듭제곱({x}, {n}) = {power_bf(x, n)}")
print(f"축소기법_ x의 n거듭제곱({x}, {n}) = {power_dc(x, n)}")
print("="*100)

# 1. k번쨰 작은 수를 찾는 문제 (k-th smallest element)
...
def quick_select(A, left, right, k):
    ...
    # k와 pos + 1 비교
    if k + left == pos + 1:  # case 1 - 피벗이 k번쨰인 경우
        return A[pos]
    elif k + left < pos + 1: # case 2 - k번쨰 작은수가 피벗의 왼쪽에 나타나는 경우
        return quick_select(A, left, pos - 1, k)
    else: 
        # case 3 - k번째 작은 수가 피벗의 오른쪽에 나타나는 경우 k를 갱신
...
# 프로그래밍 문제 1: 병합정렬 기반 중복 제거 알고리즘

#=========================================================
#  알고리즘 설계 기법 : 동적계획법 (Dynamic & Programming)
#=========================================================
# 1. fibonacci aequence (피보나치 수열) 문제
#(1) 동적계획법 - 메모제이션 방식 - top-down 방식 하향식 - 주어진 문제에서 작은 문제로 해결하는 방식

# 1. 전역 변수 메모리제이션 배열 준비 (0 ~ 10까지)
mem = [None] * 11

def fib_dp_mem(n):                  # fib(b) = ?
    if mem[n] is None:              # 아무내용이 없으면 계산
...
print("mem = 7 = ", mem[:7]) # 결과 수열 출력
print("="*100)

#(2) 동적계획법 - 테이블화 방식 - bottom-up 방식 상향식 - 작은 문제에서 큰 문제로 해결하는 방식
