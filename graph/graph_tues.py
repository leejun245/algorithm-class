...
def DFS(vtx, edge, s, visited):
    ...
    for v in range(len(vtx)):
        if edge[s][v] != 0 and not visited[v]: # 간선이 있고, 방문하지 않았다면
            DFS(vtx, edge, v, visited) # 그 정점으로 이동하여 DFS 재귀호출
...
# 3. DFS를 이용한 신장트리 생성 (인접행렬 방식) : 숙제 시험가능선 농후
def ST_DFS(vtx, edge, s, visited):
    ...
    for v in range(len(vtx)):
        if edge[s][v] != 0 and not visited[v]:
            print(f"  (간선 추가: {vtx[s]} - {vtx[v]})")   
            ST_DFS(vtx, edge, v, visited)

# 4. Prim 알고리즘 (인접행렬 방식)
# getMineVertex()를 내부함수로 정의
# INF = 999 => 간선이 없음을 의미
def MSTPrim(vtx, adj, INF = 999):
    n = len(vertex)
    ...
    for _ in range(n):
        ...
        selected[u] = True
        print(vertex[u],"()", dist[u], ")", end = ' ') # SMT에 추가된 정점 출력
        ...
        for v in range(n):
            if adj[u][v] != INF and not selected[v]:
                if adj[u][v] < dist[v]:
                    dist[v] = adj[u][v]
            print("|", vertex[v], "-", vertex[u],  "=>", dist[v], end = ' ')
        
        print(": ", dist) # 중간 dist 출력

    print("\n최종 dist 배열: ", dist)
    print("MST 총비용:",sum(dist))
