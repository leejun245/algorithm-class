...
if result != -1:
    ...
    print("패턴이 텍스트에서 발견되지 않습니다.")

# 문자열 매칭 문제 수정 : 전체 매칭 찾기
def string_matching__all(text, pattern):
    n = len(text)       # 텍스트 길이
    m = len(pattern)    # 패턴 길이
...
# 테스트 코드 
text = "ABADAADDBBDAADBDADAADDBBAADDBBDBBDABAADA"
pattern = "DAADDBB"
result = string_matching__all(text, pattern)
print(result)

#============================================================
# 2. 0/1 배낭 채우기 문제
#============================================================
def knapsack01_bf(wgt, val, W): # 모든 경우의 수 확인
    """브루트포스 방식으로 0/1 Knapsack 문제 해결"""
    # wgh: 물건 무게 히스트, val: 물건가치 리스트
    # W: 배낭 최대 무게
    ...
    bestSet = [] # 최적 조합 기록
    count = 0    # 부분 집합(조합) 번식표시용

    for i in range(2 ** n): # 0 ~ 2^n - 1 모든 부분집합 조합 탐색
        count += 1

        # 1. 각 조합에 대하여 2진수 비트 패턴 생성=> 리스트에 역순으로 저장
        s = [0] * n # 비트 스트링 초기화
        temp = i
        for j in range(n):
        ...
        chosen_items = [] # 선택된 물건 인덱스 저장 이건 책에 안나오는 변수

        for j in range(n):
            if s[j] == 1: # j번쨰 비트가 1이면 즉, j번쨰 물건을 선택한 경우
                sumWgt += wgt[j]
                sumVal += val[j]
                chosen_items.append(j) # 선택된 j번쨰 물건 인덱스 포함 책에 안나옴
        print("선택 물건 인덱스:", chosen_items, " / 총 무게 =", sumWgt, "  / 총 가치 =", sumVal)                
        
        # 배낭 무게 조건 만족하면 최대값 갱신
        if sumWgt <= W:
            print("배낭 용량 충족")
            if sumVal > bestVal:
                bestVal = sumVal # 최대 가치 갱신
                bestSet = chosen_items[:] # 최적 조합갱신 책에 안나옴
...
coins = [500, 100, 50, 10, 5, 1, 60] # 실제 최적해 : 동전 개수 3개 - (500.1), (60,2)
amount = 620
...
if coins != -1:
    ...
    print("거스름돈을 거스를 수 없습니다.")    

print("="*100)
#===================================================
# 2. 분할가능한(Fractional) 배낭문제
# 정렬 + 배낭 채우기 문제
def kanpsackFrac_greedy(weights, values, W): #무게리스트, 가치리스트, 가방무게
    ...
    items.sort(reverse = True, key = lambda x : x[0] ) # 비율기준 내림차순

    # --- 단계 3: Greedy 채우기 : O(n)---
    bestVal = 0
    bag_with_items =[] # 가방에 채워지는 물건 기록 책 X

    for ration, wgt, val, idx in items: # 비율이 높은 순서부터 물건을 채워넣기
        if W <= 0: # 배낭이 채워진 경우
            break
