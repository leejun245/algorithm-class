...
class BSTNode:
    def __init__(self, key, value):
        ...
        self.value = value      # 값 (저장된 데이터)
        self.left = None        # 왼쪽 자식 노드
        self.right = None       # 오른쪽 자식 노드

    # 출력 시 보기 좋은 형식으로 표시 (튜플 형식)
    def __repr__(self):
        return f"( {self.key!r}, {self.value!r} )"

# 2. 키(key)값으로 노드를 찾는 탐색 (순환구조)
def search_bst(n, key):
    """키(key)값으로 노드를 찾는 순환탐색 : 사전에서 "단어(키)를 찾는 것 -> 알파벳 순서로 찾기"""
    if n is None:
        return None     # 탐색 실패
    elif key == n.key:    # 키 일치
        return n
        ...
        return search_bst(n.right, key) # 왼쪽 서브트리로 재귀 탐색
    

# 3. 값(value)으로 노드를 찾는 탐색 (전위 순회 기반)
def search_value_bst(n, value):
    """value로 노드를 찾는 전위 순회 탐색: 사전에서 "뜻"을 찾는 것 -> 모든 단어의 뜻을 전부 읽어야함"""
    if n is None:
        return None
    if value == n.value:
        return n
    res = search_value_bst(n.left, value) # 왼쪽 서브트리 탐색 전위 순회
    if res is not None:
        return res
    else:
        return search_value_bst(n.right, value) # 오른쪽 서브트리 탐색


# 4. 삽입 연산 (순환 구조)
def insert_bst(root, node):
    # (1) 현재 BST트리가 비어있다면 새 노드가 그 트이의 루트가 됨
    if root is None:
        return node  # 빈 위치에 삽입
    # (2) 동인함 키가 이미 있으면 중복 삽입을 허용하지 않으므로 기존 노드 그대로 변환
    if node.key == root.key:
        return root
    # (3) 삽입할 키가 현재 루트의 키보다 작으면 왼쪽 서브트리로 재귀 삽입
    if node.key < root.key:
        root.left = insert_bst(root.left, node)
    # (4) 크면 오른쪽 서브트리로 재위 삽입
    else:
...
def bst_perf_graph_test(sizes):
    ...
    for n in sizes:
        ...
        for k, v in zip(keys, vals):
            root = insert_bst(root, BSTNode(k, v))
        end = time.time()

        insert_ms = (end - start) * 1000
        insert_times.append(insert_ms)

        test_search_keys = random.sample(keys, 100)
        start = time.time()
        for tkey in test_search_keys:
            search_bst(root, tkey)
        end = time.time()

        search_ms = (end - start) * 1000 / 100   # 1회 평균
        search_times.append(search_ms)

        print(f"[n={n}] 삽입: {insert_ms:.2f}ms, 탐색 1회 평균: {search_ms:.5f}ms")
