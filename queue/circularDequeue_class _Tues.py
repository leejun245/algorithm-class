...
class CircularDeque(CircularQueueOneSlotEmpty): # 자식(부모)
    ...
    def add_front(self, item): # 전단에서 삽입
        if self.is_full():
            ...
            self.front = (self.front - 1) % self.N # front 를 반시계방향으로 한칸 회전

    def delete_rear(self): # 후단에서 삭제
        if self.is_empty():
            raise IndexError("덱이 공백 상태 -> 삭제 불가")
        else:
            item = self.array[self.rear] # rear포인터는 마지막 요소를 가리킴
            self.array[self.rear] = None
            self.rear = (self.rear - 1) % self.N # rear 를 반시계방향으로 한칸 회전
            return item # 반환
        
    def get_rear(self): # 후단에서 검색
        if self.is_empty():
            raise IndexError("덱이 공백 상태 -> 검색 불가")
        else:
