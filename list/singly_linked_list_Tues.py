...
class Node:
    ...
    def popNext(self):
        #현재 노드 (self)의 다음 노드를 리스트에서 제거하고, 그 노드를 반환
        deleted = self.link
...
class LinkedList:
    ...
    def getNode(self, pos):
        ...
        if self.head == None: # 주어진 리스트가 공백
            ...
            ptr = self.head
            for _ in range(pos):
                if ptr == None: # pos가 리스트의 크기보다 큰 경우
                    return None
                ptr = ptr.link # 링크따라 이동
            return ptr # pos에 있는 노드 반환
    def getEnfry(self, pos):
        # 리스트의 pos 위에 있는 노드의 데이터를 반환
    ...
    def insert(self, pos, elem):
        ...
        if pos < 0: return # 자동으로 유효하지 않은 위치에 대해 ValueError 발생
        ...
        if before is None:
            if pos == 0: # 1) 머리노드로 삽입
                ...
                self.head = new
                return
            else: # 2) pos가 리스트의 범위를 벗어난 경우
                return IndexError("리스트 밖에 있는 위치")
        else: # 3) 중간노드로 삽입
