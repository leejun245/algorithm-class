...
class BTNode:
	...
	def __repr__(self): # 트리의 노드를 문자열 표현
...
def is_leaf(node): # 노드가 None이 아니고, 왼쪽과 오른쪽 자식 노드가 모두 None이 아닌경우 단말노드
	if node is None:
		return False
...
def print_leaf_nodes(node): # 이진트리의 단말노드만 출력 재귀적 정의
	...
	if is_leaf(node): # 리프노드이면 재귀호출의 종료 조건
		print(f"{node.data} is a leaf node") # 단말노드 출력
		return # 종료
	# 리프노드가 아니면 재귀함수
	print_leaf_nodes(node.left) # 왼쪽 서브노드의 단말 노드 출력
	print_leaf_nodes(node.right) # 오른쪽 서브노드의 단말 노드 출력
...
def levelorder(root): # 레벨 순회: 레벨 순으로 노드 방문 (루트 레벨 = 1, 아래 내려갈수록 레벨이 증가)
	# 파이썬 모듈 큐 이용:
	...
	while q : # 큐가 공백이 될 때까지 반복 처리
		...
		print_data(node.data) # 트리의 노드 방문(출력) - 큐에서 나오는 순서가 방문 순거를 결정
...
def count_nodes(root): # 이진트리의 총 노드 개수
	# 재귀적 정의 : 트리사 비어 있으면 0, 아니면 왼쪽과 오른쪽 서브 트리의 노드의 수 합산 + 1
...
def tree_height(root): # 이진트리의 높이 계산
	...
	return max(left_h, right_h) + 1 # 왼쪽 서브트리의 높이와 오른쪽 서브트리의 높이 중 큰값 + 루트노드높이
...
def  count_none_links(root):# 이진트리에서 노드의 개수 N 이라면 연결되지 않은 링크 수 구하기
	# 재귀적 정의: 노드가 None이면 1, 아니면 왼쪽과 오른 쪽 서브트리의 연결되지 않은 일크 수 합
...
#	링크 표현법으로 이진트리 생성 : 단말 노드 -> 루트
	...
	print("배열의 표현:", bitree_to_array(root))
	print("높이가 5인 포화이진트리:", full_binary_tree_nodes(5))
	print("높이가 5인 이진트리의 최소 노드 수:", min_nodes_in_binary_tree(5))
	print("높이가 5인 이진트리 최대 노드 수:", max_nodes_in_binary_tree(5))
